...
from recommendation.basic_IRGAN_demo.dis_model import DIS
from recommendation.basic_IRGAN_demo.gen_model import GEN
import recommendation.basic_IRGAN_demo.utils as ut
...
def main():
    print("loading model...")
    generator = GEN(ITEM_NUM, USER_NUM, EMB_DIM, lamda=0.0 / BATCH_SIZE, param=None,
                    initdelta=INIT_DELTA,
                    learning_rate=0.001)
    discriminator = DIS(ITEM_NUM, USER_NUM, EMB_DIM, lamda=0.1 / BATCH_SIZE, param=None,
                        initdelta=INIT_DELTA,
                        learning_rate=0.001)
                ...
                for d_epoch in range(100):
                    ...
                    while True:
                        ...
                        if index + BATCH_SIZE <= train_size + 1:
                            input_user, input_item, input_label = ut.get_batch_data(DIS_TRAIN_FILE,
                                                                                    index,
                                                                                    BATCH_SIZE)
                        else:
                            input_user, input_item, input_label = ut.get_batch_data(DIS_TRAIN_FILE,
                ...
                for g_epoch in range(50):
                    for u in user_pos_train:
                        ...
                        reward = sess.run(discriminator.reward,
                                          {discriminator.u: u, discriminator.i: sample})
                        reward = reward * prob[sample] / pn[sample]

                        _ = sess.run(generator.gan_updates,
                                     {generator.u: u, generator.i: sample,
